...
def ZigZag(pirate, n, dimX, dimY, x_start, y_start):
    if y_start == dimY-1:
        if x_start == 0:
            y = y_start
            List=[]            
            if n % 2 == 0:
                List.extend([((n//2)*3-2, y_start),((n//2)*3-2, y_start-1)])
                for i in range((n//2)*3-1, dimX, 2):
                    print(n,[(i,y),(i,y-1),(i,y-2)])
                    List.extend([(i,y),(i,y-1),(i,y-2),(i+1,y-2)])
                    y=y-2
                    if y+2 < 0:
                        break
                if pirate.getPosition() in List:
                    index = List.index(pirate.getPosition())
                    if index != len(List)-1:
                        x_move,y_move = List[index+1]
                        return moveTo(x_move, y_move, pirate)
                else:
                    return moveTo((n//2)*3-2, y_start, pirate)
            y = y_start
            if n % 2 == 1:
                for i in range((n//2)*3, dimX, 2):
                    print(n,[(i,y),(i,y-1),(i,y-2)])
                    List.extend([(i,y),(i,y-1),(i,y-2),(i+1,y-2)])
                    y=y-2
                    if y+2 < 0:
                ...
                if pirate.getPosition() in List:
                    ...
                    return moveTo((n//2)*3, y_start, pirate)
        if x_start == dimX-1:
            y = int(pirate.getPosition()[1])
            List=[]            
            if n % 2 == 0:
                List.extend([(dimX - 1 - ((n//2)*3-2), y_start),(dimX -1 - ((n//2)*3-2), y_start-1)])
                for i in range(dimX -1 -((n//2)*3-1), -1, -1):
                    List.extend([(i,y),(i,y-1),(i,y-2)])
                    y=y-2
                ...
                if pirate.getPosition() in List:
                    ...
                    return moveTo(dimX - 1 - ((n//2)*3-2), y_start, pirate)
            y = int(pirate.getPosition()[1])
            if n % 2 == 1:
                for i in range(dimX -1 -((n//2)*3), -1, -1):
                    List.extend([(i,y),(i,y-1),(i,y-2)])
                    y=y+2
                    if pirate.getPosition()[1] == 0:
                        break
                if pirate.getPosition() in List:
                    ...
                    return moveTo(dimX -1 -((n//2)*3), y_start, pirate)             
...
def ActPirate(pirate):
    ...
    for i in [0,dimY-1]: 
        if i != y_start: 
            y_end=i
    num_required = 2*(n//3)+(n%3)
    return ZigZag(pirate, (n%num_required) +1, dimX, dimY, x_start, y_start)
    if (
